...
public class DragSortListView extends ListView {
	...
	/**
	 * The middle (in the y-direction) of the floating View. 处于中间中间的View
	 */
	...
	/**
	 * Transparency for the floating View (XML attribute). 浮动View的透明度
	 */
	...
	/**
	 * Flag set if slide shuffling is enabled. 是否晃动
	 */
	private boolean mAnimate = false;

	/**
	 * The user dragged from this position. 拽动的起始位置
	 */
	private int mSrcPos;
			...
			if (!cancel) {
				if (remove) {
					...
					if (mDropListener != null && mFloatPos >= 0
							&& mFloatPos < getCount()) {
						final int numHeaders = getHeaderViewsCount();
						mDropListener.drop(mSrcPos - numHeaders, mFloatPos
								- numHeaders);
						Log.i("droplistener", 1 + "");
					}

					int firstPos = getFirstVisiblePosition();
					Log.i("droplistener", 2 + " firstPost:" + firstPos
							+ "mSrcPos" + mSrcPos);
					if (mSrcPos < firstPos) {
						Log.i("droplistener", 3 + "");
	...
	public boolean startDrag(int position, int dragFlags, int deltaX, int deltaY) {
		Log.i("tag", "mfloatView is =" + (mFloatViewManager == null));
